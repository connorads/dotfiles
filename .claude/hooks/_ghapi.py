...
core, and guard-mutating-api.py is the Claude delivery shell around it. Same
split as _secretpaths.py and its own delivery hooks.

Read-only `gh api` calls are auto-allowed via the `Bash(gh api *)` allow rule.
This core asks for confirmation on explicit mutating methods
...
def gh_api_decision(command: str) -> ApiDecision | None:
    ...
    Explicit writes ask for confirmation. Implicit writes are denied so the
    agent can retry with an explicit method. Read-only commands return no
    decision.

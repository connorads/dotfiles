// voxtap — Core Audio capture for vox.
//
//   voxtap record <dir> [--mic <substring>] [--no-sys]
...
//   voxtap --probe N [--mic <substring>] [--no-sys]
//                     run the same aggregate for N seconds, write nothing, report
//                     frames + elapsed + levels on stderr
//   voxtap            stream the tap alone, 48 kHz mono float32, to stdout
...
// tap needs no setup at all and captures whatever any process plays.
//
// Record mode puts the microphone and the tap in one aggregate device with the
// microphone as its clock master. One IO cycle then delivers one buffer per mic
// stream followed by one tap buffer, all for the same frames, so the two tracks
// are aligned by construction: no second clock, no padding timer, no start-order
// gate between two processes. Measured before it was built (docs/adr/0012): the
// tap buffer through silence is zeros of full size, not empty and not absent;
// with a 44.1 kHz microphone as master the tap still delivers exactly the mic's
// frame count per cycle; and a global tap follows a default-output switch on its
// own, so nothing here rebuilds on that event.
//
// The stream mode is the tap alone on a pipe, for a reader that owns its own
// clock. It pads silence to a monotonic clock on a 100 ms timer, because the
// tap delivers no callbacks at all while nothing plays when it is the only thing
// in its aggregate, and it rebuilds after a default-output change.
//
...
/// The stored track format: what mw is handed, and what the old ffmpeg path
/// wrote, so everything downstream (transcription, compact, prune, play) is
/// unchanged.
...
let usage = "usage: voxtap record <dir> [--mic <name>] [--no-sys] | --probe <seconds> [--mic <name>] [--no-sys] | --check"
...
func defaultOutputDevice() throws -> AudioObjectID {
    guard let device = readObjectID(AudioObjectID(kAudioObjectSystemObject), kAudioHardwarePropertyDefaultOutputDevice)
    else { throw fail("no default output device") }
    return device
}

func deviceUID(_ device: AudioObjectID) throws -> String {
    guard let uid = readString(device, kAudioDevicePropertyDeviceUID) else {
        throw fail("could not read device UID")
    }
    return uid
}
...
final class Capture {
    ...
    func start() throws {
        ...
        let format = try tapFormat(tap.id)
        converter = Converter(
...
// --- record: microphone + tap in one aggregate --------------------------------
...
/// selector. HAL names equal avfoundation's, so the substring vox has always
/// passed ("Microphone") resolves to the same device it did there. Resolved at
/// start, never stored: device ids and orderings change as devices come and go.
...
if case .record(let dir, let options) = mode {
...
if case .probe(let seconds, let options) = mode {
...
var defaultOutputAddress = address(kAudioHardwarePropertyDefaultOutputDevice)

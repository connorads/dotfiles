...
| A video quote matches nothing, on a video that has captions | Wrong track. `--sub-langs 'en.*'` also matches manual tracks that are not transcripts - Every Frame a Painting publishes one naming the films on screen - and picking the first by filename picked that. Read every English track into the haystack, separated so no run of words can span two of them |
...
| A quote that marks an elision never matches | `[...]`, `…` or `...`. Normalisation folds the marker away as punctuation, so the segments either side are demanded contiguous. Split on the marker and require each segment to match exactly, each starting after the previous one ended - ordered and non-overlapping, so the marker cannot become a licence to splice. How *much* was elided is not checked; whether an elision is faithful is a reading of the source, not a match |
| A podcast quote is absent from the episode page | The episode page is show notes. localfirst.fm serves the interview at `<url>/transcript`, and its own payload names a `share.transistor.fm/s/<id>/transcript.srt`. Such a transcript interleaves cue timestamps into the prose, so expect a window match rather than an exact one |
...
| A Hacker News comment is missing, or the page 429s | An `item?id=` page is the whole thread, so matching against it can confirm a *reply* as the persona's words - the adjacent-colleague signature. Read the official Firebase API instead, `hacker-news.firebaseio.com/v0/item/<id>.json`: it returns one item and names its author. HN also rate-limits CLI fetches hard and durably, which the API sidesteps |
...
| A page returns something non-empty but wrong | Bot-block, rate-limit or JS-challenge page. SlideShare's serves HTTP 200 and says only "Client Challenge" / "A required part of this site couldn't load", so a wall is scored as a missing quote unless its wording is recognised. HN throttles bursts; cxl.com, medium.com, newfangled.com and frontlines.io serve 403/429. Firecrawl gets through most of them |
| A slide quote on a video pointer matches nothing | Slide text is rendered into the video's pixels, and every route behind a YouTube URL reads speech or the description. Where the locus says `slide at MM:SS` the words were never testable, so SKIP on the locus rather than FAILing against the caption track |
...
| A book quote is absent from the publisher's page | The page is a store listing - blurb, price, contents. Leanpub's only fetchable prose is the free sample, at `s3.amazonaws.com/samples.leanpub.com/<slug>-sample.pdf` (path-style S3; the dotted-vhost form fails TLS). A sample is *part* of the artefact, so finding the words confirms them and not finding them proves nothing - a miss there must SKIP, never FAIL |

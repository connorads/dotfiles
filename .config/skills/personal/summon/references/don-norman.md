...
Donald Arthur Norman (born December 25, 1935) is an American cognitive scientist, design theorist, and author. He is the person most responsible for bringing the concept of usability out of the laboratory and into the mainstream consciousness of designers, engineers, and the general public.
...
**Nielsen Norman Group**: Co-founded with Jakob Nielsen in 1998. NN/g became the globally recognised authority on evidence-based user experience research and consulting.

**Return to academia**: Founded The Design Lab at UC San Diego in 2014 as an interdisciplinary research centre for human-centred design. Retired from UCSD in 2020 but remains an active voice in design discourse. In 2024, established the Don Norman Design Award and Summit to recognise advances in humanity-centred design.

**Key works**: *The Design of Everyday Things* (1988, originally *The Psychology of Everyday Things*; revised 2013), *Emotional Design: Why We Love (or Hate) Everyday Things* (2004), *Living with Complexity* (2010), *Design for a Better World: Meaningful, Sustainable, Humanity Centered* (2023).

**Honours**: Benjamin Franklin Medal in Computer and Cognitive Science (2006). Elected to the National Academy of Engineering (2011). Sir Misha Black Medal for Distinguished Services to Design Education (2021). CHI lifetime achievement award. Fellow of ACM, Human Factors and Ergonomics Society, Design Research Society, and the American Academy of Arts and Sciences.

## Mental Models & Decision Frameworks

This section captures the core reasoning apparatus Norman has built across five decades. These are the lenses through which he approaches every design problem.
...
Norman's two-gulf model frames every human-object interaction as bridging two gaps:

1. **Gulf of Execution**: the gap between what a person intends to do and what the system allows them to figure out. Bridged by good signifiers, constraints, mappings, and conceptual models.
2. **Gulf of Evaluation**: the gap between what the system did and how well the person understands what happened. Bridged by feedback and conceptual models.

Every design failure can be traced to one or both gulfs being too wide. The designer's job is to narrow them both.

### Seven Stages of Action

Norman's model of goal-directed behaviour, forming an action cycle divided into execution and evaluation phases:

1. **Goal** — forming an intention about what to accomplish
2. **Plan** — deciding on a course of action
3. **Specify** — determining the specific action sequence
4. **Perform** — executing the action
5. **Perceive** — observing the state of the world after acting
6. **Interpret** — making sense of what was perceived
7. **Compare** — evaluating whether the goal was achieved

This model serves as a design checklist: for each stage, ask whether the design supports the user. It also distinguishes goals from tasks — a distinction Norman insists designers chronically neglect.

### Discoverability and Understanding

Norman's two most important characteristics of good design: "Two of the most important characteristics of good design are discoverability and understanding. Discoverability: Is it possible to even figure out what actions are possible and where and how to perform them? Understanding: What does it all mean? How is the product supposed to be used?" If a design fails either test, it fails entirely.

### Conceptual Models

A conceptual model is the user's mental understanding of how a thing works. It does not need to be technically accurate — it needs to be functionally useful. The designer's job is to provide a coherent **system image** (the totality of what the product communicates) that leads the user to form an accurate conceptual model. The closer the user's mental model aligns with the actual functioning model, the more pleasant and efficient the interaction.

### Natural Mapping

The relationship between controls and their effects should be obvious through spatial correspondence or cultural convention. Norman's canonical example: stovetop burner controls laid out in the same spatial pattern as the burners themselves require no labels. Bad mapping forces reliance on memory and increases errors. Good mapping exploits "knowledge in the world" so that "knowledge in the head" becomes unnecessary.

### Knowledge in the World vs. Knowledge in the Head

Not all knowledge needed for action must be memorised. Much can be embedded in the environment through signifiers, constraints, and mappings. Designers should distribute knowledge appropriately — placing it in the world wherever possible so that users don't need to remember arbitrary procedures.

### Feedback

Users must receive clear, immediate, informative feedback about the results of their actions. Without feedback, the Gulf of Evaluation becomes impassable. Feedback must be proportionate — enough to be useful, not so much as to be annoying.

### Constraints

Norman identifies four types of constraint that guide behaviour without requiring instruction:

- **Physical constraints**: the shape of things limits what can be done (a large peg won't fit a small hole)
- **Cultural constraints**: learned conventions about acceptable behaviour (red means stop)
- **Semantic constraints**: the meaning of a situation restricts possibilities (a windscreen must face forward)
- **Logical constraints**: reasoning narrows options (if three of four screws are placed, the fourth goes in the remaining hole)

Good design uses all four to make incorrect actions difficult or impossible.

### Human-Centred Design Process

Norman's iterative cycle: **Observe, Ideate, Prototype, Test** — repeated until the design meets real human needs. Key principles:

- Focus on people and their context
- Solve the right problem (root causes, not symptoms — use the "Five Whys" technique from Sakichi Toyoda)
...
A corollary of the emotional design model. When people use something aesthetically pleasing, positive emotion makes the cognitive system more creative and more tolerant of minor problems. Ugly or frustrating things create anxiety that narrows thinking. Beauty is not opposed to usability — it actively supports it.
...
From *Living with Complexity* (2010): complexity is a natural reflection of the richness of life and is not inherently bad. Confusion — caused by poor organisation, arbitrary structure, and missing conceptual models — is the real enemy. The designer's job is to tame complexity through structure and understanding, not to eliminate it through oversimplification.
...
Borrowed from the Toyota Production System: keep asking "why?" (five times if needed) until you uncover the true underlying cause. Never accept surface-level explanations. The problem the user reports is rarely the real problem — it is a symptom.
...
Norman frequently cites Larry Tesler's insight: the total complexity of a system is a constant. Making the user's interaction simpler necessarily increases hidden complexity behind the scenes. The designer's job is to absorb complexity on behalf of the user, not to pretend it doesn't exist.
...
**Rhetorical habits**:
- Opens with a relatable frustration everyone has experienced
- Builds from particular to general, never the reverse
- Names things precisely — coining terms where existing language is imprecise
- Returns to the same examples across books (doors, stoves, taps) for continuity
- Self-corrects publicly (affordances → signifiers, usability → emotion, human-centred → humanity-centred)
- Addresses the reader directly: "Don't blame yourself"

**Hedging**: Minimal. Norman states positions as established principles, not personal opinions. When he does hedge, it is usually to acknowledge the difficulty of design rather than to qualify his own certainty.

**Humour**: Dry, observational. Not joke-telling but wry commentary on absurd design. The phrase "Norman door" amuses him — he has embraced the meme with good grace.

## Sourced Quotes

### On blaming the designer, not the user
...
— *The Design of Everyday Things*, Introduction to the 2002 Edition, p. xi
...
> "Complexity can be tamed, but it requires considerable effort to do it well. Decreasing the number of buttons and displays is not the solution. The solution is to understand the total system, to design it in a way that allows all the pieces to fit nicely together."
— *Living with Complexity* (2010)
...
Not a programmer. Norman's domain is cognitive science and design theory. His "code" is conceptual frameworks, taxonomies, and design principles. He does not publish code, maintain repositories, or express preferences about programming languages or paradigms.

However, his principles translate directly into software design:
- Affordances and signifiers → UI elements should communicate their function through appearance
- Constraints → input validation, disabled states, type systems that prevent illegal states
- Feedback → loading indicators, success/error messages, state visibility
- Conceptual models → information architecture that matches users' mental models
- Natural mapping → spatial layout of controls matching their effects
- Error design → undo, confirmation dialogs, graceful degradation
...
### A door that people keep pushing when they should pull
...
### A settings page with 47 toggles

**Problem**: A SaaS application's settings page presents users with 47 toggles, checkboxes, and dropdowns on a single page. Users report feeling overwhelmed and often leave settings at defaults even when the defaults don't suit them.

**Norman's approach**: This is a complexity-vs-confusion problem. The complexity may be legitimate — the product serves diverse needs. But presenting 47 undifferentiated options is confusion, not complexity. Apply structure: group settings by conceptual model (what the user is trying to achieve, not the system's internal architecture). Use progressive disclosure — show the most commonly changed settings first, with clear paths to advanced options. Provide meaningful defaults with explanations of what they do and why. Each setting needs feedback: what changes when I toggle this? Use constraints to prevent incompatible combinations. The seven-stage model applies: can the user form a goal (I want email notifications for X), plan an action (find the notification settings), specify it (toggle the right switch), perceive the result (see confirmation), and evaluate success (verify the change took effect)?

**Conclusion**: Don't reduce the 47 settings to 5. Organise them so that complexity becomes manageable through structure, grouping, progressive disclosure, and feedback.

### A form that users keep submitting with errors

**Problem**: An e-commerce checkout form has high abandonment. Analytics show users frequently submit with invalid data, get a generic error at the top of the page, and leave.

**Norman's approach**: Multiple design failures compounding. First, the Gulf of Execution: are required fields clearly signified? Do input constraints (format, length) have visible cues before the user acts? Second, design for error: validate inline as the user fills each field, not after submission. Make errors reversible — don't clear the entire form on failure. Third, the Gulf of Evaluation: the generic error at the top fails the feedback principle. The user cannot perceive what went wrong or where. Errors must appear adjacent to the specific field, in plain language, explaining what's needed. Fourth, conceptual model: does the form's structure match the user's mental model of the checkout process (shipping → payment → confirmation), or does it demand information in an arbitrary order? Fifth, knowledge in the world: don't make users remember format requirements — use input masks, placeholder examples, auto-formatting.

**Conclusion**: Inline validation, field-adjacent errors in plain language, input constraints visible before action, structure matching the user's mental model. The form should guide the user through imperfect approximations toward success, not punish them for inevitable errors.

### A team debating whether to simplify or add features
...
- *The Gulf of Execution yawns wide. The Gulf of Evaluation stretches beyond it. Between them, a bridge is being built from signifiers, feedback, and constraints.*

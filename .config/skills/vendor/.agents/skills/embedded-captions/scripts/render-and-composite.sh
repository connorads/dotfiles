...
for cand in "${2:-}" "${HYPERFRAMES_ROOT:-}" "$(cd "$SKILL_SCRIPT_DIR/../../.." 2>/dev/null && pwd)" "$HOME/Downloads/hyperframes"; do
	if [[ -n "$cand" && -f "$cand/packages/cli/dist/cli.js" ]]; then
		HF="$cand"
		break
	fi
done
...
if [[ -f "$PROJECT/plan.json" && -d "$PROJECT/frames_fg" ]]; then
	...
	if [[ -z "$OCC_RC" ]]; then
		OCC_RC=0
		wait "$OCC_PID" 2>/dev/null || OCC_RC=$?
	fi # capture rc without tripping set -e
	cat "$OCC_LOG"
	if ((OCC_RC != 0)); then
...
if [[ -f "$PROJECT/index_fg.html" ]]; then
	...
	rm -rf "$FG_SHADOW"
	if ((BG_RC != 0)); then
		echo "[render] bg render failed" >&2
		exit 1
	fi
	if ((FG_RC != 0)); then
		echo "[render] fg render failed" >&2
		exit 1
	fi
	...
	if ((RAIL_RC != 0)); then
		echo "[render] rail render failed" >&2
		exit 1
	fi
else
	hf_render_dir "$BG" "bg_plus_caps" "$PROJECT" ||
...
if [[ -f "$PROJECT/rail.html" ]]; then
	...
	RAIL_WEBM="$PROJECT/rail.webm"
	[[ -f "$RAIL_WEBM" ]] || {
		echo "[render] rail.webm missing (parallel rail pass failed?)" >&2
		exit 1
	}

...
Send feedback only when the user explicitly asks for that report. If telemetry is disabled or the user opted out, do not send it. Search results, a completed render and tool warnings are not requests to send feedback. Prepare and send the report below only after that request.<!-- LOCAL PATCH (connorads dotfiles): Render-report instructions and retries require an explicit user request and respect for opt-outs. -->
...
The `hyperframes feedback` command soft-warns when a non-10 `--comment` is missing `REPRO COMMAND:`, and when a rating-≤-7 visual-defect comment is missing `COMPOSITION_STRUCTURE:`. The warnings print above the submission ack and do not block — some legitimate reports (a one-line "cloudrun quota bumped yesterday, fine now") won't fit the mold. Do not resend to silence a warning. Explain it locally; send a corrected report only on a further explicit user request, respecting opt-outs.

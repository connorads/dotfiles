...
1. Run `<skill-base-dir>/scripts/impeccable context` once per session, where `<skill-base-dir>` is the directory that contains this SKILL.md (the skill folder, not a plugin root two levels above it); keep cwd at the user's project. That base directory resolves every `.agents/skills/impeccable/scripts/impeccable <verb>` command in this skill and its references, and `.agents/skills/impeccable/scripts` is the fallback only when the runtime reports no base directory. On a Windows shell without `sh`, call `.agents/skills/impeccable/scripts/impeccable.cmd` instead. The launcher uses an existing engine only; it never downloads one. No Node runtime is required for the engine. Pass a named source file or route as `--target <path>`. It loads PRODUCT.md, DESIGN.md, the matching surface brief, and native-platform guidance when applicable; follow its directives and do not rerun it.
...
<!-- LOCAL PATCH (connorads dotfiles): Catalogue skill works without an engine; engine installation and maintenance are separate tasks -->

**Launcher unavailable:** Read existing PRODUCT.md and DESIGN.md directly, without inventing missing context, then continue with the applicable design references and available tools. Do not install an engine or retry engine-dependent steps. Replace engine-driven context, direction-selection, font and comp helpers with direct inspection and the runtime's existing tools; say which automated checks did not run. Detector, hook administration, live-browser and shortcut commands require a separately provisioned engine: report that requirement instead of claiming they ran. Keep cwd at the user's project and resolve bundled files from this skill's actual base directory.

The visual-polishing budget does not replace required repository checks or permit known functional failures to be left unresolved. Close disposable browser sessions after verification; never close unrelated sessions.

<!-- LOCAL PATCH (connorads dotfiles): Catalogue helper definitions are instructions, not registered agent types -->

...
`Railway.Website.TanStackStart` deploys a
[TanStack Start](https://tanstack.com/start) app to Railway. TanStack
Start is pure Vite, so Alchemy runs your project's own `vite build`:
the SSR server plus client assets run on one
...
app. It builds for workerd, not Node, so a Railway deploy needs it
gone:
...
See the
[TanStack Start API reference](/providers/railway/website/tanstackstart)

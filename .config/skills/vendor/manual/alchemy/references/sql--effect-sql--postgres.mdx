...
Hyperdrive's `connectionString` and `Fly.ConnectPostgres`'s
`connectionString` both resolve at runtime, so either slots
straight in:
...
- [Fly Postgres](/fly/data/postgres) — the same client on a Fly
  Service via `ConnectPostgres`.

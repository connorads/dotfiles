...
| `Ctrl+b Alt+Shift+W` | worktree picker: repo (only when more than one), PR verdict (`✓ reap`/`✓ merged`/`○ open`/`✗ closed`/`· -`/`? …`), local flags (`◉ live`/`● dirty`/`↑ahead`/`↓behind`), then branch (last, so only it truncates) + git log/status preview below the list; enter→focus/open window, ctrl-v→pane here, ctrl-x→remove (refused if pane open or dirty; merged branch deleted, unmerged kept), alt-R→reap merged (all repos; warns about open panes) |
...
| `Ctrl+b Alt+v` | record (vox) - start/stop; starting prompts for a title at once while the capture starts behind it (Enter keeps, a title renames, Esc discards), stopping transcribes in the background |

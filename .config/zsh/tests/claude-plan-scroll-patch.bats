...
PLAN_PATCH="$FUNCTIONS_DIR/claude/claude-plan-scroll-patch"
MARKER_REL=".cache/claude-plan-scroll-patch.stale"

setup() {
  setup_test_home
  mkdir -p "$HOME/.cache" "$HOME/.config/zsh/functions"
  ln -s "$FUNCTIONS_DIR/patch" "$HOME/.config/zsh/functions/patch"
  NEEDLE='ky.jsx(pW,{ref:ht,flexDirection:"column",height:At,stickyScroll:!1,children:zt})'
  PATCHED='ky.jsx($ ,{ref:ht,flexDirection:"column",height:void 0,            children:zt})'
}

@test "needle and replacement keep the packed payload length stable" {
  [ "${#NEEDLE}" -eq "${#PATCHED}" ]
}
...
@test "--reapply patches an unpatched bundle and clears the stale marker" {
  ...
  run_zsh_function "$PLAN_PATCH" --reapply "$HOME/claude"
...
@test "--reapply re-patches after a --restore" {
  ...
  run_zsh_function "$PLAN_PATCH" --restore "$HOME/claude"
  [ "$status" -eq 0 ]
...
@test "--reapply warns and writes a marker when the needle is gone, exiting 0" {
  printf 'prefix ky.jsx(planMode_RESHAPED,{children:zt}) suffix' >"$HOME/claude"

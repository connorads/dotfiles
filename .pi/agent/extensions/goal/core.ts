...
// the self-driving truth table, completion auditing, token metering, and the
// rendered prompt blocks. No pi imports, no I/O, no clock — every function here is
// deterministic so the whole decision surface is unit- and property-testable in
// isolation (functional core; the imperative shell lives in index.ts/runtime.ts).
//

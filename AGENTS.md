...
| [tmux/scripts/agent-state.sh](./.config/tmux/scripts/agent-state.sh)    | Hook-driven agent status: per-pane `@agent_state` (`blocked>done>working>idle`, seen-bit done→idle), window dots, outer-terminal bell on blocked. Agents: wait on this option, don't scrape - see the `tmux` skill (`skl`), references/coding-agents.md. Subsystem docs in [.config/tmux/AGENTS.md](./.config/tmux/AGENTS.md) |
| [tmux/strategies/](./.config/tmux/strategies/)                          | Resurrect agent-session restore: Claude/Codex/OpenCode panes resume their own conversations (`--resume <id>`) with saved-argv fidelity. Already built - do not re-implement. Subsystem docs in [.config/tmux/AGENTS.md](./.config/tmux/AGENTS.md) |
...
ccp [-y] [<name>|personal]  # launch Claude Code on an account (bare = fzf picker; -y = cy flags: system-append + skip-perms); real names + 2-char aliases in ~/.zshrc.local

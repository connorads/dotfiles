...
agent hibernate [<target>] [--force]  # kill an idle Claude pane to reclaim RAM/swap and park a thawer in it; the conversation resumes in full on thaw. Only idle/done go without --force (refusal = exit 6)
...
**How `up` works**: by default it *bumps* both lockfiles and commits each change (symmetric with the nix flake model) - `mise upgrade` within ranges (4-day quarantine, selected tools exempt), which auto-locks **every** `lockfile_platforms` entry for each changed tool plus current-platform provenance (so no separate `mise lock` refresh is needed), then `dotfiles commit` the lock if it changed - unless `mise upgrade` itself exited non-zero, in which case the lock commit is skipped and the lock left dirty (a partial failure still rewrites it, so committing would pin versions nothing can install; reverting would discard every other tool's legitimate bump). `up` carries on and still exits 0 - the warning is the signal. Updates brew/apt; `nfu`; rebuilds; commits `flake.lock` only once that rebuild succeeded. Same rule as `mise.lock` and for the same reason: a lock nothing on this host can build must not reach git, or every later `up` and every other host pulling it reproduces the failure. A failed rebuild leaves the lock dirty rather than reverting it, so the bump is never silently discarded. **`--frozen` / `-s` / `--skip-flake`** is the *frozen* path: bump nothing - converge tools to the committed `mise.lock` via `mise install`, skip brew/apt, skip the flake bump/commit - then rebuild. Use it to reproduce a known-good toolchain (e.g. on a fresh Linux box).

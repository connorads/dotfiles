...
**aube** (primary npm backend for mise): config at `~/.config/aube/config.toml`. Set via mise: `npm.package_manager = "aube"`. Since mise v2026.7.15 that value selects aube **embedded as a library** ([jdx/mise#11149](https://github.com/jdx/mise/pull/11149) landed the embedding in v2026.7.12); the shell-out-to-the-CLI path is named `aube_cli`. The setting kept its value and changed its meaning, so "aube the setting value" and "aube the command" have diverged: the `aube` binary on `PATH` is still the real CLI, pinned at 1.38.0, and it is no longer what installs mise's npm tools. Anything driving that CLI against a mise install dir is addressing a store the embedded library does not use - it brands the virtual store `node_modules/.mise` while the CLI looks for `.aube`, and silently drops the work at `tracing::debug!`. Layered defenses beyond a simple age gate:
...
- `trustPolicy = "no-downgrade"` (default) - fails install if a package's trust evidence weakens (e.g. previously had SLSA provenance, now doesn't). What it flags in practice: `@mariozechner/clipboard-*` platform packages lost provenance after a CI refactor at 0.3.3 (root pkg kept it - same publisher, same npm signing key), so they carried a `trustPolicyExclude` until upstream restored attestations; provenance is back, so the exclude is gone and `no-downgrade` enforces them again
...
**Rule: aube keys live in `~/.config/aube/config.toml` only, never spelled into `~/.npmrc`.** From mise v2026.8.14 the per-install config moves to `<install>/.config/aube/config.toml` and sits **below** `userNpmrc` in aube's precedence order, so an aube-spelled key in `~/.npmrc` would silently override the per-install settings for every npm-backed tool. The two files are not interchangeable and the precedence is not the one you would guess.

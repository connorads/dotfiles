...
- similar-name check (default, aube >= 1.38) - refuses a package whose name is within a small edit distance of a top-100,000 npm name (`ERR_AUBE_SIMILAR_PACKAGE_NAME`). Interactive it prompts `y/N`; **non-TTY it is a hard refusal**, and mise's npm backend shells out to `aube add` non-interactively - so a false positive fails `up` closed

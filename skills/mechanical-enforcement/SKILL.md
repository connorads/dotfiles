...
| Every environment variable the app reads is declared | A committed `.env.schema` (varlock) with `@defaultSensitive=true`, and `varlock load && varlock audit` in the check command | An undeclared or drifted variable: `load` fails the gate when the environment does not satisfy the schema, `audit` fails it when code reads a name the schema does not declare |
...
| No `--no-verify`, and no other hook-manager kill switch | Project AGENTS.md for the human lane; for the agent lane a pre-execution tool gate that returns a deny or ask - Claude Code `PreToolUse`, pi `tool_call`, Codex `PreToolUse`, opencode `tool.execute.before`. `references/agent-tool-gates.md` | Bypassing the whole gate. Preventable for an agent's shell tool, never for a human terminal; the flag is one of eight spellings a correct policy enumerates |
...
| Gating the agent's own tool calls: per-harness hook contracts, ask versus deny, the VCS bypass surface, testing a guard | `references/agent-tool-gates.md` |

...
| Nix | nixfmt | deadnix + statix | `nix eval` over every host config's `.drvPath` | - | Evaluation is the real correctness gate; the linters catch the dead code evaluation cannot. Disable statix's `repeated_keys` - it fights idiomatic flat nix-darwin/home-manager attributes. See `references/nix.md`. |
...
| Every dependency build script has a recorded decision, not just a working local install | Custom pre-commit checker reading installed manifests against the allowlist config (pnpm: `allowBuilds` in `pnpm-workspace.yaml`); glob the step on `package.json`/lockfile/workspace YAML. Ready-made: the hk skill's `assets/pnpm-build-scripts-check.mjs` | A CI-only install failure (`ERR_PNPM_IGNORED_BUILDS`) that no local run can surface, because a global `ignoreScripts` masks the package manager's own check - and the reproduction command is itself a de-protection. Assert the decision statically; never run the install to find out |

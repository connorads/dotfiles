...
| README renders on PyPI | `twine check --strict dist/*` | A `long_description` that fails to render | README rendering only, despite the name: a 3.4 MB sdist that had swallowed 13 unrelated wheels passes it. `--strict` is required - without it a package with no README at all exits 0 "PASSED with warnings". |

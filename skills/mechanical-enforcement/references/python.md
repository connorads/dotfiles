...
- Prefer **pytest-archon** over PyTestArch, which derives module identity from the filesystem path rather than the importable package name and could not be made to fail a positive control. Rejected: `pydeps` (a Graphviz visualiser that exits 0 regardless, printing nothing at all when Graphviz is absent); `pycycle` (prints `No worries, no cycles here!` on a real two-file cycle).

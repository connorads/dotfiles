...
|---|---|---|---|
| Lockfile matches every manifest | `pnpm install --frozen-lockfile`, spelled explicitly in CI | `package.json` edited without re-locking, so CI resolves something the author never saw | `ERR_PNPM_OUTDATED_LOCKFILE`, exit 1. Specifier-level only: corrupting all 227 `integrity: sha512-` values exits 0. |
| Nothing left to dedupe | `pnpm dedupe --check` | Two copies of one dependency that a single install would collapse | A strict superset of the frozen check: it also fails on an out-of-sync lock, under the wrong label. Read-only; exit 1. |
...
- **`--lockfile-only` cancels the CI frozen default and re-locks in silence.** `CI=true pnpm install` on a drifted tree exits 1; `CI=true pnpm install --lockfile-only` on the same tree exits **0**, goes to the network and rewrites the lock (verified 2026-09-03 against pnpm 11.20.0). So `--frozen-lockfile` must be spelled out; the `CI=true` default is not a substitute, and `--lockfile-only` is a specifier diff rather than the drift gate.
...
- **api-extractor exits 0 against a stale `dist`.** Editing a source file and running the gate without rebuilding reports "completed successfully" at exit 0; rebuilding first exits 1 on the report diff. The `&&` is the gate. `--local` also exits 0 on the `ae-missing-release-tag` warnings that make plain `run` exit 1, so CI must never pass it. Verified 2026-09-03 against @microsoft/api-extractor 7.59.0.

...
- **A suite that reads a build output passes against yesterday's artefact with zero signal.** The file exists, every assertion passes, and no runner key can tell that `dist/` predates the source. Declare the build-to-test edge in the task runner (turbo `dependsOn: ["build"]`, an hk step order, a make prerequisite), not in a README step. The `testing` skill states the same hazard for goldens generated from build output; this is the general case.
...
| Tests enter through the harness, never a hand-built runner | An authored `jsPlugins` rule that returns `{}` from `create` unless `context.filename` matches `*.test.*` / `*.spec.*`, then reports the `CallExpression` of the raw entry point: `Effect.run*` / `ManagedRuntime.make`, a bare `asyncio.run`, `app.listen`, a hand-built DI container | A test that opts out of the setup and teardown the harness owns (scope, test clock, layer memoisation), where a forgotten `await` passes silently | Scope by filename inside `create`, not by an `overrides[].files` glob, so a relocated test keeps the rule; name it `"error"` in `rules`, since an unlisted jsPlugin rule reports nothing. `jsPlugins` is alpha: canary it per `typescript.md`, Gate integrity. Authoring conventions: `typescript.md`, Authoring a custom rule. |

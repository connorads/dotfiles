...
// allowlisted is denied rather than prompted), and an allowlist of read tools
// plus read-only git subcommands.
...
export const claudeArgv = (model: string, effort: string): string[] => [
  "claude",
  "-p",
  "--setting-sources",
  "",
...
export const runClaude: RunReviewer = async ({ spec, prompt, cwd }) => {
  const r = await run(claudeArgv(spec.model, spec.effort), { cwd, stdin: prompt });
  let out: Obj;
  try {
